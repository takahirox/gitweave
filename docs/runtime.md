...
GitWeave remains a thin runtime, not a general credential or environment sandbox. The assigned worktree defines official artifact state, and publication remains the responsibility of explicit Graph/System Actions. Environment inheritance does not grant agents permission to publish. Any future environment restriction should address a concrete observed problem with the smallest necessary change.
