...
- Each node invocation attempt produces exactly one checkpoint commit, which is both its artifact state and its stable execution identity. The attempt ref and Git note attach to that commit. Same-tree (empty) checkpoints are intentional: a node that changes no files, or only causes external side effects, still records that it ran and completed at that graph boundary, so they must not be optimized away. These checkpoints make Runs inspectable and analyzable later and are the intended boundary for future resume/restart; resume itself is not implemented (see below).
