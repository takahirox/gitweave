...
PR and Issue modes use one persistent bare repository per GitHub repository, `.gitweave/repos/OWNER/REPO.git` under the invoking directory, shared by all Runs of that repository. The path is lowercased because GitHub names are case-insensitive, so `Owner/Repo` and `owner/repo` share a store; the Run still records `github_repository` as passed to `--repo`. The first Run creates the store, initializing it beside the target and renaming it into place so concurrent first Runs cannot collide. Automatic `git gc` and maintenance are disabled in the store, because they could run while other Runs are writing; run `git -C .gitweave/repos/OWNER/REPO.git gc` manually when no Runs are active if you want to repack. An existing path that is not a bare repository (for example an empty directory left by manual cleanup) fails the Run instead of falling through to an enclosing checkout. Later Runs reuse it, so fetches transfer only objects that are not already present, and disk use grows with new objects rather than one full copy per Run. All per-Run state is namespaced (`refs/gitweave/<run-id>/…` and `refs/notes/gitweave/<run-id>`), so each Run's record, attempts and notes stay separately inspectable, and provenance persistence pushes only the Run's own refs. The CLI includes the store's absolute `repository` path in its output; use `git -C PATH show RUN_REF:run.json` to inspect a Run. The store is retained, including when execution fails, and is not automatically deleted. No local clone is required, and no existing checkout is modified by initialization. Stores created by earlier versions under `.gitweave/runs/<run-id>/repository.git` are left as they are and remain inspectable; there is no migration.
...
GitWeave remains a thin runtime, not a general credential or environment sandbox. The assigned worktree defines official artifact state. Task-specific behavior, including any GitHub operation, is defined by node instructions or commands. Any future environment restriction should address a concrete observed problem with the smallest necessary change. The runtime Git wrapper's environment handling is unchanged. Worktrees share a Git object store, and concurrent Runs of the same GitHub repository share one store. Attempt worktrees get unique directory names, and GitWeave serializes `git worktree add`/`remove` across processes with an exclusive lock file (`gitweave-worktrees.lock` in the repository's Git directory, including a local `--commit` checkout's `.git`), because Git does not make concurrent worktree changes in one repository safe; GitWeave does not require Docker or disable native permission checks.
