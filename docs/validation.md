...
On 2026-09-25, `python3 -m unittest discover -s tests -v` passed all 111 deterministic tests; `git diff --check` passed. CLI tests run `--issue` and `--commit` Runs without a request and verify `Runtime` receives `None`; existing tests still pass a request through unchanged. Runtime tests verify that an omitted request is `null` in `run.json`, every node context, and the Run-base input message, that a supplied request reaches nodes unchanged, and that a non-text request is rejected. The Agent preamble describes `request` as optional operator guidance.
